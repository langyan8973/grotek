...
<c:set var="pageTitle" value="生产用工" scope="request" />
    ...
    <div class="content">     
        <div class="container-fluid">
        	<ul class="nav nav-tabs" style="margin-top:20px">
			    ...
			    <li class="active"><a href="${laborUrl }">生产用工</a></li>
			</ul>  
            <div id="pad-wrapper" class="users-list">
                ...
                <div class="row-fluid table">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                        <!-- row -->
                        <c:forEach items="${laborcosts}" var="laborcost">
                        	<tr>
                        		<td>
                        			<c:url var ="proUrl" value = "/manager/productboxes/profile?id=${laborcost.box.id }"></c:url>
	                                <a href="${proUrl }" class="name">${laborcost.box.name }</a>
	                            </td>
	                            <td>
	                                ${laborcost.box.specification }
	                            </td>
	                            <td>
	                                ${laborcost.pielaborPrice }
	                            </td>
	                            <td>
	                                ${laborcost.tonlaborPrice }
	                            </td>
	                            <td>
		                            <fmt:formatDate value='${laborcost.date }'
...
function search(){
	var text = $("#searchtext").val();
		window.location.href = '<c:url value="laborcost" />?context='+encodeURIComponent(text);
}
