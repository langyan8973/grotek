...
<html>
<head>
	<title><c:out value="工时费用统计" /></title>
...
<body style="background-color:#ffffff;">
        ...
        <div id="pad-wrapper" class="user-profile">
            ...
            <div class="row-fluid header boxbottomline">
            	<c:if test="${employee != null}">
            		<div class="span4">
                     <h3 class="name">${employee.fullname }工时费用统计</h3>
                 	</div>
	...
	<script>
		...
		function search() {
			var eid = $('#eid').val();
			var url = '<c:url value="service.json" />';
			...
			$.get(url, function(result) {
				...
				$('#title1').html('为经销商工作的时间');
				$('#title2').html('为经销商工作的费用');
				...
				var body=$("#tablebody");
				$.each(result[2], function (n, value) {
					var row;
					row = $("<tr></tr>");
					var td = $("<td rowspan='"+value.count+"'></td>");
					td.append(value.dname);
					row.append(td);
					$.each(value.items, function (i, item){
						if(i==0){
							var td1 = $("<td></td>");
							td1.append(item.type.name);
							row.append(td1);
							var td2 = $("<td></td>");
							td2.append(item.content);
							row.append(td2);
							var td3 = $("<td></td>");
							td3.append(item.total);
							row.append(td3);
							var td4 = $("<td></td>");
							td4.append(item.stime+"---"+item.etime);
							row.append(td4);
							body.append(row);
						}
						else{
