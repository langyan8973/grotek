...
<html>
<head>
	<title><c:out value="员工收入总和" /></title>
...
<body style="background-color:#ffffff;">
        ...
        <div id="pad-wrapper" class="user-profile">
            ...
            <div class="row-fluid header boxbottomline">
            	<c:if test="${employee != null}">
            		<div class="span4">
                     <h3 class="name">${employee.fullname }收入统计</h3>
                 	</div>
            ...
            <div class="row-fluid profile">
            	...
            	<div class="span7">
                   ...
                   <div id="canvas-holder" style="text-align:center;">
						<canvas id="chart-area" width="700" height="450" />
					</div>
               	</div>
            </div>
        </div>
	...
	<script>
		$(document).ready(function() {
			$('#starttime').val(moment(new Date()).subtract(366, 'day').format("YYYY-MM"));
		...
		function search() {
			var eid = $('#eid').val();
			var url = '<c:url value="totalincome.json" />';
			...
			$.get(url, function(result) {
				...
				myPieChart1 = new Chart(ctx1).Pie(result[2]);
				$("#legend1").html('');
				...
				$('#title1').html("总收入为：￥"+result[3][0]+"万元");
			});
		}
	</script>
</body>
