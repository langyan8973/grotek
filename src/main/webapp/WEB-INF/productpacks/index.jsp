...
<link rel="stylesheet" href="${cssUrlp }" type="text/css" media="screen" />
            ...
            <div id="pad-wrapper" class="users-list">
                ...
                <div class="row-fluid table">
                    <table class="table table-bordered">
                        ...
                        <!-- row -->
                        <c:set var="i" value="0" />
                        <c:forEach items="${productpacks}" var="productpack">
                        	<tr>
                        		<td>
                        			<c:url var ="proUrl" value = "/manager/productpacks/profile?id=${productpack.id }"></c:url>
	                                <a href="${proUrl }" class="name">${productpack.name }</a>
	                            </td>
	                            <td>
	                                ${productpack.code }
	                            </td>
	                            <td>
	                                ${productpack.type.name }
	                            </td>
	                            <td>
	                                ${productpack.price }
	                            </td>
    ...
    <div class="modal hide fade" id="addOne-modal" data-backdrop="static" data-keyboard="false">
		<c:url var="addproductpackUrl" value="addone" />
		<form id="addproductpack-form" class="form-horizontal" action="${addproductpackUrl }" method="post" enctype="multipart/form-data">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				<h3 id="myModalLabel">新增包材</h3>
			</div>
			<br>
			<fieldset>
...
function addProductPackFormValidate() {
	$("#addproductpack-form").validate(
			{
				debug : true,
				rules : {
					name : {
